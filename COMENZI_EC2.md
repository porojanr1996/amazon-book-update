# Comenzi pentru Repornire Servicii pe EC2

## Opțiunea 1: Rulează scriptul automat

```bash
cd /Users/testing/books-reporting
bash fix_ec2_services.sh
```

## Opțiunea 2: Comenzi manuale (copy-paste)

### 1. Salvează cheia SSH

```bash
cat > /tmp/ec2_key.pem << 'EOF'
-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
EOF
chmod 600 /tmp/ec2_key.pem
```

### 2. Conectează-te și repornește serviciile

```bash
ssh -i /tmp/ec2_key.pem ec2-user@16.171.115.88
```

### 3. Pe EC2, rulează:

```bash
cd /home/ec2-user/app/books-reporting
export GOOGLE_SHEETS_SPREADSHEET_ID=1-y5ly84oAV1GkhpLlD3MfvLZSi-5UThypHXsmch6RiA

# Oprește serviciile
sudo docker-compose -f docker/docker-compose.yml down

# Repornește serviciile
sudo docker-compose -f docker/docker-compose.yml up -d

# Verifică statusul
sudo docker ps

# Verifică logs-urile
sudo docker logs docker-sheets-service-1 --tail 20
sudo docker logs docker-scraper-service-1 --tail 20
```

### 4. Testează aplicația

```bash
curl http://16.171.115.88:5001/health
```

Sau deschide în browser: `http://16.171.115.88:5001`

